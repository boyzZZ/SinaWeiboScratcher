 ...
 *
 */
public class ParseTransWeiboLimited implements WeiboParseProcessor {

	@Override
	public void parse(Element e) throws Exception {
		/**
		 * 步骤：
		 * 获取元素，然后将值传给TransArticle对象
	     */
	    TransArticle article=new TransArticle();
	    article.setAid(GlobalVar.getNumber(3));
	    Element firstDiv=e.children().get(0);
	    if(firstDiv.getElementsByClass("cmt").get(1).getElementsByTag("a").size()==0){
	    	//处理原文已删的微博
	    ...
	    int org_praiseAmt=Integer.parseInt(firstDiv.getElementsByClass("cmt").get(2).text().substring(2, firstDiv.getElementsByClass("cmt").get(2).text().length()-1));
	    int org_transAmt=Integer.parseInt(firstDiv.getElementsByClass("cmt").get(3).text().substring(5, firstDiv.getElementsByClass("cmt").get(3).text().length()-1));
